...
class VarListNE inherits VarList {
  ...
  head()  : Variable { x };
  tail()  : VarList { rest };
  init(y : Variable, r : VarList) : AUTO_TYPE { { x <- y; rest <- r; self; } };
...
class LambdaList {
  ...
  headC() : AUTO_TYPE { { abort(); new Lambda; } };
  headN() : AUTO_TYPE { { abort(); 0; } };
  tail()  : AUTO_TYPE { { abort(); new LambdaList; } };
...
class LambdaListRef {
  ...
  add(env : VarList, c : Lambda) : AUTO_TYPE {
    {
