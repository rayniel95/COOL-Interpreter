...
class A {
   ...
   method2(num1 : AUTO_TYPE, num2 : AUTO_TYPE) : AUTO_TYPE {  -- plus
      (let x : AUTO_TYPE in
	 {
            x <- num1 + num2;
	    (new B).set_var(x);
   ...
   method3(num : AUTO_TYPE) : AUTO_TYPE {  -- negate
      (let x : AUTO_TYPE in
	 {
            x <- ~num;
	    (new C).set_var(x);
   ...
   method4(num1 : AUTO_TYPE, num2 : AUTO_TYPE) : AUTO_TYPE {  -- diff
            if num2 < num1 then
               (let x : AUTO_TYPE in
		  {
                     x <- num1 - num2;
   ...
   method5(num : AUTO_TYPE) : AUTO_TYPE {  -- factorial
      (let x : Int <- 1 in
	 {
	    (let y : AUTO_TYPE <- 1 in
	       while y <= num loop
	          {
                     x <- x * y;
	             y <- y + 1;
