...
class Board inherits IO { 
 ...
 rows : AUTO_TYPE;
 columns : AUTO_TYPE;
 board_size : AUTO_TYPE;

 size_of_board(initial : AUTO_TYPE) : AUTO_TYPE {
   initial.length()
 };

 board_init(start : String) : SELF_TYPE {
   (let size :AUTO_TYPE  <- size_of_board(start) in
    {
	if size = 15 then
	 {
	  rows <- 3;
...
class CellularAutomaton inherits Board {
    ...
    cell(position : AUTO_TYPE) : AUTO_TYPE {
 ...
 north(position : AUTO_TYPE): String {
 ...
 neighbors(position: Int): AUTO_TYPE { 
 	{
